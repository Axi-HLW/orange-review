...
11. 借机会问Kafka八股文。
12. 借机会问ES八股文。
13. Canal 监听 MySQL 是一张表还是多张表？
- > 答：监听多张表，因为Canal监听的是binlog，因此一个canal可以监听多张表的变更，每张表的变更会被发送到不同的topic

14. 你的项目是否涉及到跨表的事务？
- > 答：没有涉及到跨表的事务。为啥不能涉及，因为你canal把不同表的更新写入到不同的topic中，原本binlog自带的事务边界，在kafka这一层就被破坏了，如果还想实现事务，就要在消费者端写一个事务协调器，这比较复杂。
- 发评论，就是向评论表插入一条评论。
- 发起申诉，就是向申诉表插入一条申诉。申诉表通过评论ID关联评论表。
- 回复评论，就是向评论回复表插入一条数据。

- O端审核评价需要查询评论申诉表 + 评论表，我的应对策略是评论申诉表不维护评论状态，对于申诉成功需要隐藏的评论，直接根据ID去查评论表，然后把评论表中的数据隐藏掉即可。
